...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const mod = [ownerNumber,"6289655478810@s.whatsapp.net","6282198571732@s.whatsapp.net","6282387804410@s.whatsapp.net","62895801181650@s.whatsapp.net","6285752164908@s.whatsapp.net","6288296158460@s.whastapp.net"]
			const adminbotnumber = ["6289655478810@s.whatsapp.net"]
			const frendsowner = ["6282387804410@s.whatsapp.net"]
			const premium = ["6289655478810@s.whatsapp.net","6285807107404@s.whatsapp.net","6289655478810@s.whatsapp.net","62895801181650@s.whatsapp.net","6282198571732@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
					...
					case 'nobg':
                                        ...
                                        ranp = getRandom('.png')
                                        rano = getRandom('.webp')
                                        anu1 = await fetchJson(`https://api.vhtear.com/removebgwithurl?link=${teks}&apikey=Mantanagung1`, {method: 'get'})
...
if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
				...
				if (!isPublic) return reply(mess.only.public)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/nhentaipdfdownload?query=287167&apikey=Mantanagung1`)
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result.pdf_file)
					client.sendMessage(from, buffer, document, {mimetype: 'document/pdf', quoted: mek})
					break
				case 'randomcat':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  XPTN BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
